...
def song(_, message):
    ...
    try:
        ...
        m.edit("❌ خطاء من فضلك تحدث مع المطور")
        print(e)
...
async def vsong(client, message):
    ...
    try:
        ...
        print(e)
    try:
        msg = await message.reply("📥 **رفع الفيديو...**")
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("» **هذا هي كلمات الاغنية.**")
            return
        query = message.text.split(None, 1)[1]
        rep = await message.reply_text("🔎 **تم البحث...**")
        resp = requests.get(
